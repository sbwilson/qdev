...
- source_spec: `docs/bmad/implementation-artifacts/spec-hex-allocators-and-invariant-coverage.md`
  ...
  evidence: With the allocator pure, the window between "resolve the set" and "write the file" is explicit and unsynchronised. Harmless today — `create story` is the only production caller and it holds the advisory write lock across both, and the hex pair has no caller at all — but `architecture.md`'s rule reads as a guarantee about ids, not about a locked section, so the note now says so. A reservation protocol (or documenting the lock as part of the allocation contract) is the fix. Review by: epic 2 story 2.8, or the first allocator caller that writes outside the write lock.
- source_spec: `docs/bmad/implementation-artifacts/spec-hex-allocators-and-invariant-coverage.md`
